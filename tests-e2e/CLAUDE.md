...
- **The look tier is two lists, one per engine.** The **field** list is every lab preset at 300/1400/2600ms on the wall clock, the shipping renderer's baselines. The **cast** list is every lab preset at 700/1150/2200ms, read off R-01's beat clock instead: 700ms is charge _content_ (the ambient medium, with the spell's own manifestation still to come), 1150ms is inside the strike, 2200ms is mid-body of a 5s cast. Two more cast cases run `column-balanced` under the `crystal` and `aeroform` sigils, so the only difference between those baselines and the plain one is the look row. 78 baselines: 36 field, 42 cast.
- **Look baselines live in `golden-look.e2e.ts-snapshots/`.** Regenerate with `npm run test:golden:look:update` after any deliberate visual change, and eyeball the diff before committing. That command rewrites both lists; scope it with `-g "look: cast-"` when only the cast engine moved, so the field baselines stay byte-identical. If a baseline is missing its case skips with a hint instead of failing. Field cases allow a 2% pixel diff because the field effect jitters with `Math.random`; cast cases allow 0.5%, because the cast engine is seeded end to end and reads no clock. The primary gate is the motion tier (`npm run test:golden`, see [`../tests/CLAUDE.md`](../tests/CLAUDE.md)); this tier only catches what pixels can say and motion cannot.
- **A look frame is driven, never waited on.** `?preset=<id>&frameMs=<n>` on `/tools/spell-effect-lab` loads a preset and steps the preview to that timestamp on a scripted clock, then stops and stamps `data-golden-frame` on the effect canvas. `&engine=cast` drives the redesign's cast engine instead of the default `field` one, and `&sigil=<id>` picks the look row, narrowed against the lab's own sigil list so a URL cannot select one that does not exist. The hook is test-only and lives in [`../src/routes/tools/spell-effect-lab/lab-goldens.ts`](../src/routes/tools/spell-effect-lab/lab-goldens.ts). The spec also seeds `Math.random` through `addInitScript`, because field particle spawning uses it and an unseeded page paints a different frame every run.

...
- **The look tier is one list, because there is one engine.** Every lab preset at 700/1150/2200ms, read off R-01's beat clock: 700ms is charge _content_ (the seal's ink igniting and the ambient medium, with the spell's own manifestation still to come), 1150ms is inside the strike, 2200ms is mid-body of a 5s cast. Two more cases run `column-balanced` under the `crystal` and `aeroform` sigils, so the only difference between those baselines and the plain one is the look row. 63 baselines, all named `cast-*`.
...
- **The effect canvas is WebGL, so reading pixels back takes asking.** A drawing buffer is discarded at composite unless the page requested otherwise, and `getContext('2d')` on that canvas returns null. `SpellCanvasPage.goto` therefore loads `/?castReadback=1` ([`../src/lib/cast/stage/readback.ts`](../src/lib/cast/stage/readback.ts)), which is the only thing the page under test asks the app for that production does not; the look tier's scripted clock turns the same flag on for itself. `helpers/castProbe.ts` then reads the stage's own context with `readPixels`.

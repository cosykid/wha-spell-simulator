...
- **A look frame is driven, never waited on.** `?preset=<id>&frameMs=<n>` on `/tools/spell-effect-lab` loads a preset and steps the preview to that timestamp on a scripted clock, then stops and stamps `data-golden-frame` on the effect canvas. **The clock is walked at sixty frames a second, never jumped**, for both engines: a baseline is a picture of a frame a display could show, and both carry state a frame builds on — classic steps a particle system per call, the stage accumulates a trail whose deposits are capped per call. A stage baseline costs several seconds because of it. `&sigil=<id>` picks the look row, narrowed against the lab's own sigil list so a URL cannot select one that does not exist. The hook is test-only and lives in [`../src/routes/tools/spell-effect-lab/lab-goldens.ts`](../src/routes/tools/spell-effect-lab/lab-goldens.ts).

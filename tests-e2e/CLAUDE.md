...
- **A look frame is driven, never waited on.** `?preset=<id>&frameMs=<n>` on `/tools/spell-effect-lab` loads a preset and steps the preview to that timestamp on a scripted clock, then stops and stamps `data-golden-frame` on the effect canvas. The hook is test-only and lives in [`../src/routes/tools/spell-effect-lab/lab-goldens.ts`](../src/routes/tools/spell-effect-lab/lab-goldens.ts). The spec also seeds `Math.random` through `addInitScript`, because particle spawning uses it and an unseeded page paints a different frame every run.

...
export function sampleVelocity(n: Nozzle, pos: Vec3, out: Vec3): number {
	...
	if (n.manifests) {
		...
		if (n.Q > 1e-3) {
			...
			const tx = dx0 - ox * axial;
			const ty = dy0 - oy * axial;
			const tz = dz0 - oz * axial;
	...
	// ---- swirl: circulation Γ — a rising vortex column (§13 pinwheel) -------
	addVortex(out, px, h, pz, rho, n.gamma, F, CONFIG.U_COLUMN);
...
export function sampleAmbientVelocity(n: Nozzle, pos: Vec3, out: Vec3, throttle = 1): void {
	...
	// The capacitor throttle gates ONLY this channel: the grasp stores what the
	// sink collects, so a charged grasp stops inhaling — but the twist and the
	// conveyor move matter without storing it and keep acting (§12 stirred-cup:
	// the stirring never dies). A near-tangential seal (C≈0, tiny capacitor)
	// would otherwise charge instantly and kill its own pinwheel.
	if (Math.abs(pull.C) > 1e-3 && (pull.C < 0 || throttle > 0)) {
		let vx = px;
		...
		// Inflow (C > 0) is what charges the grasp, so it alone is throttled.
		const gate = pull.C > 0 ? throttle : 1;
		const mag = -CONFIG.U_PULL * pull.C * F * gate * Math.exp(-dDisk / (CONFIG.L_PULL * n.reach));
		out.x += vx * mag;
		out.y += vy * mag;
		out.z += vz * mag;
	}

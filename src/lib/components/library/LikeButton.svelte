...
<script lang="ts">
	interface Props {
		/** Tally shown beside the thumb. */
		count: number;
		/** Whether this reader has liked the spell. */
		liked: boolean;
		onLike: () => void;
	}

	let { count, liked, onLike }: Props = $props();
</script>
...
<button
	...
	aria-pressed={liked}
	aria-label={liked ? 'Remove your like' : 'Like this spell'}
	onclick={onLike}
>

<script lang="ts">
  ...
  function analyze() {
    ...
    const recStatus = analysis.recognition?.recognitionStatus ?? (analysis.matches.length ? "unknown" : "valid");
    setStatus(
  ...
  <main class="workspace maker-workspace detector-lab-workspace">
    <section class="canvas-panel maker-canvas-panel">
      <div class="toolbar detector-lab-toolbar">
        ...
        <select class="select-control" bind:value={referenceId} disabled={!paperOverlay} onchange={analyze}>
            ...
            {#each analysis.matches.slice(0, 8) as match, index (match.entry.id)}
              ...
              <article class="reference-card detector-match-card {index === 0 ? 'best' : ''}">
                ...
                <div class="detector-match-body">
                  ...
                  <dl>
                    <div><dt>Template</dt><dd>{percent(match.templateMatch.confidence)}</dd></div>
                    <div><dt>Ink</dt><dd>{percent(match.templateMatch.inkScore)}</dd></div>
                    <div><dt>Explained</dt><dd>{percent(match.templateMatch.candidateExplainedRatio)}</dd></div>
                    <div><dt>Covered</dt><dd>{percent(match.templateMatch.templateCoveredRatio)}</dd></div>
                    <div><dt>Rotation</dt><dd>{Math.round(match.templateMatch.rotationDeg)} deg</dd></div>
                  </dl>
                </div>
              </article>
            {/each}

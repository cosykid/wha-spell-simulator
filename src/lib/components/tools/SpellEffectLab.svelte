<script>
  ...
  import { activePortalPlane, convergenceFlow, resetParticleState } from "$lib/renderer/effects/effectUtils.js";
  ...
  async function copyIR() {
    const json = JSON.stringify(roundDeep(buildSpellIR({ values, element, activatedAt, config: CONFIG })), null, 2);
    irInput = json;
    try {
  ...
  <main class="workspace maker-workspace effect-lab-workspace">
    <section class="canvas-panel maker-canvas-panel">
      <div class="toolbar effect-lab-toolbar">
        ...
        <button type="button" onclick={() => { restartSpell(); setStatus("Particles reset", "prepared"); }}>
          Reset Particles
        </button>
      </div>
    ...
    <aside class="side-panel effect-lab-side-panel">
      ...
      <section class="diagnostic-block">
        <h2>Paste IR</h2>
        <p class="effect-lab-help">Apply IR loads values from pasted JSON. Copy Current IR writes the sliders back into JSON for testing.</p>
        <textarea class="template-output effect-lab-ir-input" spellcheck="false" bind:value={irInput}></textarea>

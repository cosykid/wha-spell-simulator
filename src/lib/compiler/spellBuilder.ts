...
function sameKindAlternateConfidence(recognition: Recognition): number {
	return (
		recognition.diagnostics?.topMatches?.find(
			(score) => score.kind === recognition.kind && score.id !== recognition.id
		)?.confidence ?? 0
	);
}

function invalidSpell(status: string, glyphAST: GlyphASTLike, warnings: string[] = []): SpellIR {
...
function calculateSpellGravity(manifestationInfluence: Record<string, number>): number {
	return clamp(
		1 - (manifestationInfluence.levitation ?? 0) * PHYSICS_TUNING.levitationGravityScale
	);
}

function manifestationSignature(manifestations: Record<string, Manifestation>): string {
	return Object.entries(manifestations)
		.map(([id, manifestation]) => {
			const point = manifestation.point
				...
				: '';
			const radius =
				manifestation.radius === undefined ? '' : `.r${Math.round(manifestation.radius * 100)}`;
...
function calculateSpellDuration({
	...
	return clamp(
		SPELL_PARAMETER_TUNING.durationMinSeconds +
			Math.pow(durationScore, SPELL_PARAMETER_TUNING.durationCurve) *
				SPELL_PARAMETER_TUNING.durationSecondsScale,
		SPELL_PARAMETER_TUNING.durationMinSeconds,
...
export function compileSpell({
	...
	if (glyphAST.ring.unsupportedMultipleRings?.length) {
		return invalidSpell('Multiple rings detected', glyphAST, [
			GLYPH_WARNINGS.unsupportedMultipleRings
		]);
	}

	if (glyphAST.unsupportedMultipleSigils?.length) {
		return invalidSpell('Multiple sigils detected', glyphAST, [
			GLYPH_WARNINGS.unsupportedMultipleSigils
		]);
	}
	...
	if (!SUPPORTED_ELEMENTS.has(primary.element)) {
		return invalidSpell('Unsupported element', glyphAST, [

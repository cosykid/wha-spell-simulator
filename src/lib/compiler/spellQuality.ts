...
import type { AppConfig, GlyphAST, Recognition } from '../types.js';
...
function topMatchCompetitorConfidence(recognition: Recognition | null): number {
	return (
...
export function calculateSpellQuality(glyphAST: GlyphAST): number {
	...
	const insideScore =
		1 - Math.min(1, (glyphAST.unknowns?.length ?? 0) / QUALITY_TUNING.unknownSoftLimit);
...
export function calculateSpellStability(glyphAST: GlyphAST, config: AppConfig): number {
	...
	const boundaryPenalty = (glyphAST.warnings ?? []).includes(GLYPH_WARNINGS.symbolNearLayerBoundary)
		...
		: 0;
	const centerPenalty = (glyphAST.warnings ?? []).includes(
		GLYPH_WARNINGS.centerUnknownContamination
	)
		? STABILITY_TUNING.centerPenalty
		: 0;
	const inverseInstability =
		1 - (glyphAST.globalMetrics?.instability ?? STABILITY_TUNING.instabilityFallback);

	return clamp(
		ringNeatness * STABILITY_TUNING.ringNeatness +
			(symbolNeatness || STABILITY_TUNING.symbolNeatnessFallback) *
				STABILITY_TUNING.symbolNeatness +

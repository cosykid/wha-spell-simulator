...
import type { Manifestation, Recognition, Semantic, SemanticDeltas, Vector } from '../types.js';

const DELTA_TARGETS = ['force', 'focus', 'spread', 'range', 'lifetimeBias'] as const;

const INWARD_DIRECTION_OFFSET_DEG = 180;

interface SignShapeDeltas {
	force: number;
	focus: number;
	spread: number;
	range: number;
...
interface WeightedSign {
	sign: Recognition;
	influence: number;
}

interface ManifestationGroup {
	id: string;
...
interface ManifestationAggregate {
	primaryManifestation: string;
	manifestations: Record<string, Manifestation>;
	manifestationInfluence: Record<string, number>;
}

interface SurfaceDirection {
	x: number;
	y: number;
	strength: number;
}
...
function manifestationId(sign: Recognition): string {
	return sign.semantic?.manifestation ?? (sign.id as string);
}

function signDirection(sign: Recognition): Vector {
...
function signShapeDeltas(sign: Recognition): SignShapeDeltas {
	const shape: Record<string, number> = sign.shape ?? {};
	...
	return {
		...
		focus: clamp(
			(elongationNorm - SIGN_SHAPE_TUNING.focusElongationOffset) *
				SIGN_SHAPE_TUNING.focusElongationScale,
			0,
		...
		directionWeight: clamp(
			(axisDominance - SIGN_SHAPE_TUNING.directionAxisOffset) *
				SIGN_SHAPE_TUNING.directionAxisScale,
			0,
...
export function signInfluence(sign: Recognition): number {
	...
	return clamp(
		sign.confidence * sign.neatness * sizeWeight * lengthWeight * layerWeight * distanceWeight
	);
}

function convergenceProfile(weightedSigns: WeightedSign[], strength: number): ConvergenceProfile {
...
function signDirectionWeight(sign: Recognition): number {
...
export function aggregateManifestations(signs: Recognition[]): ManifestationAggregate {
	if (!signs.length) {
		return {
	...
	const groups = new Map<string, ManifestationGroup>();
	signs.forEach((sign) => {
	...
	const manifestations: Record<string, Manifestation> = Object.fromEntries(
	...
	return {
		...
		manifestations,
		manifestationInfluence: Object.fromEntries(
			sortedGroups.map((group) => [group.id, group.totalInfluence])
		)
	};
}

export function combineSignDirection(signs: Recognition[]): SurfaceDirection {
	...
	const strength = clamp(
		magnitude / Math.max(SIGN_INFLUENCE_TUNING.minimumDirectionMagnitude, vector.weight)
	);
	return {
...
export function aggregateSemanticDeltas(signs: Recognition[]): SemanticDeltas {
	return signs.reduce<SemanticDeltas>(
		(sum, sign) => {
			const influence = signInfluence(sign);
			const semantic: Semantic = sign.semantic ?? {};
			const featureDeltas = signShapeDeltas(sign);

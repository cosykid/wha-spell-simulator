...
export class LightEffect extends ElementEffect<LightParticle, LightFlowConfig> {
	...
	protected baseParticleCount(
		...
		return (34 + spellIR.force * 38) * (0.78 + flow.scale * 0.28);
	}
	...
	protected spawnParticle(
		...
		const sideJitter = randomBetween(-0.025, 0.025) * flow.scale * flow.mod.jitterMul;
		...
		return {
			...
			speed,
			radius: randomBetween(4.4, 8.6) * (0.86 + flow.scale * 0.2),
			phase,
			travel: 0,
			age: 0,
	...
	protected updateParticle(particle: LightParticle, flow: LightFlowConfig, dt: number): void {
		particle.age += dt;
	...
	protected drawParticle(
		...
		this.traceTrail(ctx, particle.trail, flow, particle.phase);
		ctx.stroke();

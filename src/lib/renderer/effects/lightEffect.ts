...
function lightFlowConfig(spellIR: RenderSpellIR, ring: RingInfo, portal: Portal, frame: number): LightFlowConfig {
...
function spawnLightParticle(spellIR: RenderSpellIR, ring: RingInfo, portal: Portal, flow: LightFlowConfig): LightParticle {
...
function traceLightTrail(ctx: CanvasRenderingContext2D, trail: Vector[], flow: LightFlowConfig, phase: number): void {
  if (!trail?.length) {
    return;
  }

...
function waterFlowConfig(spellIR: RenderSpellIR, ring: RingInfo, portal: Portal, frame: number): WaterFlowConfig {
...
function randomPortalSource(portal: Portal, flow: WaterFlowConfig): Vector {
...
function spawnSuspendedWaterParticle(spellIR: RenderSpellIR, portal: Portal, flow: WaterFlowConfig, frame: number): WaterParticle {
  const angle = Math.random() * Math.PI * 2;
...
function spawnWaterParticle(spellIR: RenderSpellIR, ring: RingInfo, portal: Portal, flow: WaterFlowConfig, frame: number): WaterParticle {
  if (flow.suspended) {
...
function updateSuspendedWaterParticle(particle: WaterParticle, flow: WaterFlowConfig, dt: number): void {
  particle.age += dt;
...
function updateWaterParticle(particle: WaterParticle, flow: WaterFlowConfig, dt: number): void {
  if (flow.suspended) {
...
function projectWaterParticle(particle: WaterParticle, flow: WaterFlowConfig): Vector {
  const base = {
...
function drawWaterMass(ctx: CanvasRenderingContext2D, projected: Vector, particle: WaterParticle, flow: WaterFlowConfig, alpha: number): void {
...
function drawWaterCore(ctx: CanvasRenderingContext2D, projected: Vector, particle: WaterParticle, flow: WaterFlowConfig, alpha: number): void {
  const heightRatio = clamp(particle.height / Math.max(1, flow.maxHeightHint));
...
function drawWaterHighlight(ctx: CanvasRenderingContext2D, projected: Vector, particle: WaterParticle, alpha: number): void {
...
function visibleWaterParticle(
  particle: WaterParticle,
  flow: WaterFlowConfig,
  spellIR: RenderSpellIR
): { projected: Vector; alpha: number } | null {
  const alpha =
...
export function drawWaterEffect(
  ...
  while (state.particles.length < targetCount) {
    state.particles.push(spawnWaterParticle(spellIR, ring, portal, flow, state.waterFrame as number));
  }

  const visibleParticles: Array<{ particle: WaterParticle; projected: Vector; alpha: number }> = [];
  for (const particle of state.particles) {
    updateWaterParticle(particle as WaterParticle, flow, dt);
    const visible = visibleWaterParticle(particle as WaterParticle, flow, spellIR);
    if (visible) {
      visibleParticles.push({ particle: particle as WaterParticle, ...visible });
    }
  }

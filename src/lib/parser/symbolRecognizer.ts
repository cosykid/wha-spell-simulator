...
function aspectCompatibility(candidateRatio: number, templateRatio: number, rotationDeg: number): number {
...
function strokeLengthProfile<T>(strokes: T[], pointGetter: (stroke: T) => { x: number; y: number }[]): number[] {

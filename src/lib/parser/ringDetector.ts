...
import type { TopologyResult } from "./topologicalFloodFill.js";
import type { AppConfig, RingGap, RingInfo, Stroke, UnsupportedRing, Vector } from "../types.js";
...
interface RingCandidate extends MeasuredRing {
  score: number;
}
...
function fitCircle(points: Vector[]): Circle | null {
  if (points.length < 8) {
    return null;
  }
...
function fallbackCircle(strokes: Stroke[]): Circle {
...
function measureOpenCoverage(strokes: Stroke[], circle: Circle, config: AppConfig) {
...
function measureRing(
  ...
  const residual = topology?.closed ? topology.normalizedRmse ?? 0 : stddev(radialDistances) / Math.max(1, radius);
  ...
  const roundness = topology?.closed ? topology.perfection ?? 0 : referenceRing?.roundness ?? fittedRoundness;
...
function collectOpenRingStrokes(seedRing: MeasuredRing, strokes: Stroke[], config: AppConfig): Stroke[] {
...
function strokeAngularCoverage(stroke: Stroke, circle: Circle, config: AppConfig) {
  const bins: boolean[] = new Array(RING_BIN_COUNT).fill(false);
  const metrics = strokeCircleMetrics(stroke, circle, config, bins);
...
function pruneRedundantShortRingStrokes(ringStrokes: Stroke[], circle: Circle, config: AppConfig): Stroke[] {
...
function collectTopologicalRingStrokes(strokes: Stroke[], topology: TopologyResult, config: AppConfig): Stroke[] {
...
function addTopologicalCandidate(candidates: RingCandidate[], strokes: Stroke[], config: AppConfig): TopologyResult {
...
function buildOpenRingCandidates(strokes: Stroke[], config: AppConfig): RingCandidate[] {
...
function addReferenceFilteredClosureCandidate(
...
function bestOpenRingCandidate(openCandidates: RingCandidate[]): RingCandidate | null {
  return (
    [...openCandidates]
...
export function detectRing(strokes: Stroke[], previousRing: RingInfo | null, config: AppConfig): RingInfo {
  const candidates: RingCandidate[] = [];
  ...
  candidates.sort(
    (a, b) => Number(b.complete) - Number(a.complete) || b.score + b.radius * 0.001 - (a.score + a.radius * 0.001)
  );

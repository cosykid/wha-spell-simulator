...
import type { AppConfig, RingInfo, Stroke, Vector } from '../types.js';
...
function normalizePoint(point: Vector, ring: RingInfo) {
...
export function classifyStrokesAgainstRing(strokes: Stroke[], ring: RingInfo, config: AppConfig) {
	if (!ring.found) {
	...
	return strokes.map((stroke) => {
		...
		const insideRatio =
			normalized.filter((point) => point.radiusNorm < config.layers.outerMax).length /
		...
		const outsideRatio =
			normalized.filter((point) => point.radiusNorm > config.layers.boundaryMax).length /
			Math.max(1, normalized.length);
		...
		return {
			...
			boundaryRatio,
			usedByParser,
			canJoinSymbol: usedByParser || (classification === 'boundary-near' && outsideRatio <= 0.08)
		};
	});
}

export function summarizePolar(point: Vector, ring: RingInfo, config: AppConfig) {

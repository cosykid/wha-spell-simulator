...
function asPointArray(stroke: Point[] | { points?: Point[] }): Point[] {
	return Array.isArray(stroke) ? stroke : (stroke.points ?? []);
}

function resampleStroke(points: Vector[], targetCount: number): Vector[] {
...
function roundPoint(point: Vector, digits: number): Vector {
...
export function normalizeStrokesForTemplate(
	...
	return {
		sourceAspectRatio:
			Math.round(
				(bounds.width / Math.max(options.fitToBounds ? 0.0001 : 1, bounds.height)) * 1000
			) / 1000,

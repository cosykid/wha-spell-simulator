...
function normalizedRotationMagnitude(degrees: number): number {
	const normalized = normalizeAngleDeg(degrees);
	return Math.min(normalized, 360 - normalized) / 180;
}

function rotatePoint(point: Vector, transform: RotationTransform | null): Vector {
	if (!transform) {
		return point;
	}
...
function templateInk(strokeTemplate: StrokeTemplate): InkLayers {
...
function candidateInk(candidate: SymbolCandidate, rotationDeg: number): InkLayers {
...
function compareInk(candidateInk: InkLayers, referenceInk: InkLayers) {

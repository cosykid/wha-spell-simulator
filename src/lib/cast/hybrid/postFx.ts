...
/** Fraction of the accumulation the bloom is taken at. */
...
export class PostFx {
	...
	resize(width: number, height: number) {
		const w = Math.max(2, Math.round(width * POST.trailScale));
		const h = Math.max(2, Math.round(height * POST.trailScale));
	...
	/**
	 * The accumulation as it already stands, composited out to the canvas. A
	 * frame that advanced no step has nothing new to deposit, and fading the
	 * buffer for it would make the trail a function of the display's rate rather
	 * than of the cast's clock.
	 */
	present() {
		this.#composite.uniforms.uTrail.value = this.#trailA.texture;
		this.#composite.uniforms.uBloom.value = this.#bloomA.texture;

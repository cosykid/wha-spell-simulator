/**
 * @file The four brush stamps, painted once into a single 2x2 atlas: a dry
 * streak, a flame lick, a soft wash and a soot smudge.
 *
 * The stamps are luminance masks (white on transparent) and the layer tints them
 * per mark from `palette.ts`, so four stamps carry the entire pigment ramp of
 * every look row. Each one meets a lumpy organic mask before it lands, so no
 * stamp ever carries energy to its quad border and the mass can never show a
 * rectangle.
 *
 * Which cell a mark reaches for is decided on the CPU, in `brushSlot.ts`, so a
 * cast can be performed in plain Node with no canvas to paint on.
 */

import * as THREE from 'three';
import { mulberry32, type Rng } from '../rng.js';
import { between, bunched, CELL, crowd, ink, MID, organicMask, sweep } from './brushBristles.js';
import type { Spine } from './brushBristles.js';

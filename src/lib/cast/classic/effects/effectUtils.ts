/**
 * @file The classic engine's shared particle helpers: the render-time IR, the
 * particle shapes, the `SpellIR`-derived scalars, and the convergence, dispersion
 * and bolt manifestations every element effect reshapes its emission with.
 *
 * Restored from `src/lib/renderer/effects/effectUtils.ts` at `b439a01`. The one
 * block that did not come back is its copy of the portal constants: the portal's
 * numbers and its projection live in `$lib/portal/portal.js` now, and are
 * re-exported here so the effect files read the same names they always did.
 */

import { clamp, normalizeVector, perpendicularVector } from '../../../utils/geometry.js';
import { projectSealDirection, type Portal } from '../../../portal/portal.js';
import type { ClassicTuning } from '../tuning.js';
import type { SpellField } from '../field/spellField.js';
import type { SpellIR, Vector } from '../../../types.js';
...
/**
 * The compiled `SpellIR` augmented with what the engine works out per frame and
 * spreads into a copy before handing it to an effect:
 * - `emission` (0..1) holds the effect back through the portal tilt and fades it
 *   out at end-of-duration.
 ...
 * - `field` is the classic force field, derived from `SpellIR.reading`. The IR
 *   itself has carried no field since the redesign.
 */
export type RenderSpellIR = SpellIR & {
	emission?: number;
	portalFit?: number;
	field?: SpellField;
};
...
/** Per-effect mutable render state, owned by the engine and reset per signature. */
...
export function scaledParticleCount(
	...
	return Math.min(classic.particleCap, Math.round(baseCount * effectOpacity(spellIR)));
}
...
/**
 * The spell's paper-local direction as a screen-space one. Positive z points out
 * of the paper toward the top. The projection is the portal's own, so a beam and
 * the paper it leaves stay in one perspective; the legacy copy here foreshortened
 * height by 1.0 where the fitted camera implies 0.898.
 */

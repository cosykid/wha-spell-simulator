// Field-driven effect: one renderer for every element, used whenever the seal
// carries field sources. Particles live in seal space, spawn where the domain
// allows, and advect through the superposed sign forces — leaning columns,
// angled-pull vortices, and on-ring droplets all emerge from the field sum
// rather than per-element special cases.
//
// Restored from `src/lib/renderer/effects/fieldEffect.ts` at `b439a01`. The one
// number that changed is the particle height scale (see PARTICLE_SHAPE).
...
import { HEIGHT_FORESHORTENING } from '../../../portal/portal.js';
import type { ElementId, RingInfo } from '../../../types.js';
...
const PARTICLE_SHAPE = {
	...
	minSpawnHeight: 0.02,
	maxSpawnHeight: 0.12,
	// Screen height of one seal-space z unit, as a fraction of the portal's x
	// radius. The portal's own, not a hand-picked 0.8: a particle rising off the
	// paper and the paper it rose from have to share one elevation.
	heightScale: HEIGHT_FORESHORTENING,
...
export function drawFieldEffect(
	...
	const baseCount =
		classic.particleBaseCount * (0.7 + clamp(field.domain.strength) * 0.2) + spellIR.force * 80;
	const targetCount = scaledParticleCount(baseCount, spellIR, classic);

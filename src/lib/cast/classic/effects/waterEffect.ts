...
import type { RingInfo, Vector } from '../../../types.js';
...
function spawnSuspendedWaterParticle(
	spellIR: RenderSpellIR,
	portal: Portal,
...
function updateSuspendedWaterParticle(
	...
	const targetLateral =
		particle.homeLateral! + Math.sin(particle.phase + particle.age * 0.034) * flow.suspendedWander;
	const targetDepth =
		particle.homeDepth! +
...
export function drawWaterEffect(
	...
	const targetCount = Math.round(
		scaledParticleCount(baseCount * (0.66 + scale * 0.22), spellIR, classic) *
			boltBurst(state.waterFrame as number, flow.mod.bolt)
	);

	while (state.particles.length < targetCount) {
		state.particles.push(
			spawnWaterParticle(spellIR, ring, portal, flow, state.waterFrame as number)
		);
	}

...
- **The cast is heard beside whichever engine draws it.** `renderCanvasFrame` hands the same `(spellIR, ring, timestamp)` to a [`CastSound`](../../cast/sound/CLAUDE.md) after the engine, and the sound is compiled from the score, so it survives a style swap and neither engine knows it exists. `ui.soundEnabled` is a kept preference like `effectStyle` (the `SoundToggle`, or `M`), and muting is a ramp on the master rather than a skipped schedule, so unmuting lands mid-cast. The audio context is created on the first gesture the runtime's `mount()` hears, never on load.
...
- **The ML runtime loads on the first stroke, not on mount.** `loadDictionary` still spawns the classifier worker eagerly, because the template recognizer inside it is small and has to answer the moment ink lands, but that worker comes up with its ML side unloaded: the runtime behind `recognizeCandidatesHybridMl` is roughly 25 MB of weights and wasm, and a visitor who reads the page and leaves never needs it. `onStrokeStart` in `#setupInputControllers` is the one seam that asks for it, through `recognition.warmMlRefinement()`, once per page. Nothing earlier counts as intent to draw, and nothing later is needed: a reading that arrives before the download settles (a placed shape, a loaded preset, a stroke drawn faster than the fetch) refines a beat late instead of never, because [`../../parser/ml/hybrid.ts`](../../parser/ml/hybrid.ts) loads the runtime on the recognition path too, and a runtime that never arrives leaves the template verdict standing.

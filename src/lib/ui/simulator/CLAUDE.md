...
- **Sealing the ring locks freehand input.** `summary.canvasLocked` (in [`../spellSummary.ts`](../spellSummary.ts)) turns true once the ring is complete, or when the structure is unsupported. An active seal is a cast in progress, not an editing surface. Erase or undo is the way back, not a new unlock path — with one ruled exception: once the cast is over (`recognition.castSpent`, a timer armed off `summary.castEndsAt`), a primary tap in draw mode tears the page off through `actions.freshPage`, which is a clear rather than an unlock, so the seal itself stays inviolable while a spell can still perform. The tearing tap never inks, because the paper is still tilted at that instant and `canvasPointFromEvent` maps through a flat rect. `recognition.resetSpellState()` is what makes any wipe land in the same tick: it drops the compiled spell so lock, tilt, and status fall with the ink instead of waiting out a classify pass.

...
export function buildSpellIR({
  ...
  const { effectScale, force, spread, focus, gravity, duration, stability } = values as Required<ControlValues>;
  const direction = directionFromTiltAngles(values.xTiltDeg!, values.yTiltDeg!) as SpellIR["direction"];
...
export function valuesFromSpellIR(
  ...
  if (ir.direction) {
    if (typeof irDirection.xTiltDeg === "number" || typeof irDirection.yTiltDeg === "number") {
      ...
      set("xTiltDeg", Math.atan2(Number(irDirection.x ?? 0), Number(irDirection.z ?? 1)) * (180 / Math.PI));
      set("yTiltDeg", Math.atan2(Number(irDirection.y ?? 0), Number(irDirection.z ?? 1)) * (180 / Math.PI));
    }
  }

...
import { PAPER, RING_INK } from './brushPalette.js';

/** Where the ring sits on the canvas, as the lab places it. */
export const RING_CENTER_Y = 0.56;
 ...
 *
 * @example
 * drawPaperPlate(paperCanvas, 0.34);
 */
export function drawPaperPlate(canvas: HTMLCanvasElement, ringRadiusNorm: number): void {
	...
	inkRing(ctx, width / 2, height * RING_CENTER_Y, Math.min(width, height) * ringRadiusNorm, rng);
}

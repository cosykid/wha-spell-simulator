...
// Where a parcel is born. Three mouths: the seal disc that carries the mass, the
// three drawn column sites that feed tongues into it, and the charge beat's
// motes drifting in from outside the ring.
Spawn spawnParcel(bool mote) {
	...
	// The punch is vertical. Spending it sideways would lay the column out flat.
	float rise = speed * (0.42 + 0.95 * draw(10.0)) * (1.0 + 1.3 * uStrike) * (1.0 + 0.3 * lick);
	if (feeder < 0.22 && uSiteCount > 0.5) {
		...
		float radius = uFootprint * sqrt(r0) * (1.0 + 0.15 * uStrike);
		// The strike arrives as a domed front, fastest and highest on the axis, so it
		// tears upward instead of rising as one flat slab.
		float core = 1.0 - r0;
		float lead = uStrike * (0.12 + 0.95 * core) * draw(9.0);
		s.pos = vec3(dir * radius, 0.005 + 0.05 * r1 + lead);
		s.vel = vec3(dir * speed * (0.06 + 0.3 * uStrike) * r2, rise * (1.0 + 0.6 * uStrike * core));
	}
	s.life = (uLife + uLifeSpread * r1) * (1.0 + 0.85 * lick) * (1.0 - 0.2 * uStrike);
	return s;
}
...
void main() {
	...
	// The pinch. The column narrows with height, then flares where it is spent,
	// which is what turns the top into tongues rather than a nozzle.
	// No flare at the top: the pinch runs all the way, so the column tapers to
	// licks instead of stalling into a cap. The boundary itself wanders, which is
	// what keeps the silhouette off a clean cylinder wall.
	float target = uFootprint * (1.0 - uNarrow * smoothstep(0.0, 0.95, hn)) + 0.04;
	target *= 1.0 + 0.52 * vnoise(vec3(pos.xy * 1.5, uTime * 0.6 + pos.z * 1.3));
	...
	float gain = uTurbulence * (0.48 + 1.4 * smoothstep(0.0, 0.55, hn) + 0.55 * uStrike) * (0.5 + 0.7 * t01);
	vel += turb * gain * uDt;
...
void main() {
	...
	if (dead) {
		...
		if (rollFire(gate)) {
			Spawn s = spawnParcel(false);
			// Stagger the strike's ages so the punch is not one flat tone.
			gl_FragColor = vec4(s.pos, draw(8.0) * 0.55 * s.life * uStrike);
			return;
		}
		if (rollMote(gate)) {
	...
	if (pos.z > uReach * 1.45 || length(pos.xy) > 1.6) {
		age = life;
	}

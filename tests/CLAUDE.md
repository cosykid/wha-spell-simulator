...
- [`castVolume.test.ts`](castVolume.test.ts) — the substrate's pure core: the per-element behavior matrix, the chip-law numbers, tracer replay determinism, water's pool against earth's mound and heap, the ring attractor's hollow, the gather's containment and the shared pool, and the canon physics laws (the element's grip, the excluded volume, rigidity under focus, crystal setting in the air, the light beam, the pooled sink), each tagged with the ground-truth section it pins. The marching-cubes skin is a pure function of this state and the look tier owns its pixels.
...
- [`castSound.test.ts`](castSound.test.ts) — the sound layer's pure core: voice-row resolution, and when a cast is heard (R-01 silence before the strike, R-02's stretch, silence by `totalMs`, seeded grains). How it sounds needs a browser and is not asserted here.

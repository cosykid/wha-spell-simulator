 ...
 * against: cast state and plan text. Run it only when a plan, score, cell or
...
import { CAST_DIR, LAB_PRESETS, castFileName, renderPresetCast } from './casts.js';
...
for (const preset of LAB_PRESETS) {
	const castName = castFileName(preset.id);
	writeFileSync(join(CAST_DIR, castName), renderPresetCast(preset));
	casts.add(castName);
...
const stale = pruneStale(CAST_DIR, '.txt', casts) + pruneStale(PLAN_DIR, '.txt', plans);

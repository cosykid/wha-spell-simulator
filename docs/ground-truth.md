> Salvaged wholesale from branch `theorycrafting` (theorycrafting/GROUND_TRUTH.md)
> as the spec of intent, per the salvage register in
> [`animation-redesign.md`](animation-redesign.md). Its wiki image captures
> (`theorycrafting/wiki/images/`) remain on that branch; image links below
> resolve only there.

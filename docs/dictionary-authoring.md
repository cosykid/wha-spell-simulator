...
- `src/lib/dictionary/sigils/*.ts`
- `src/lib/dictionary/signs/*.ts`
- `src/lib/dictionary/sample-spells.json`

Sigils and signs are split one entry per file, written as TypeScript modules (`export default { ... } satisfies SigilEntry;`) so a malformed entry fails type checking instead of breaking at runtime. The numeric filename prefix controls dictionary order; the `id` field inside the file remains the stable symbol identifier and must match the filename after the prefix (enforced by `tests/dictionary.test.ts`).

Adding the file is all that is required: each folder's `index.ts` is auto-generated by `scripts/generate-dictionary-indexes.ts`, which runs automatically before `dev`, `build`, `check`, and `test:unit` (or manually via `npm run gen:dictionary`). The dictionary test fails if an index is stale.
...
1. Add a new entry file to `src/lib/dictionary/sigils/NN-example-sigil.ts` (the `id` must match the filename after the `NN-` prefix).
...
9. Test in the main app with Diagnostics enabled (`npm run dev` regenerates the folder index automatically).
...
1. Add a new entry file to `src/lib/dictionary/signs/NN-example-sign.ts` (the `id` must match the filename after the `NN-` prefix).
2. Choose a stable `id`.
3. Set `displayName`.
4. Set `allowedLayers`.
...
10. Test in the main app with Diagnostics enabled (`npm run dev` regenerates the folder index automatically).
